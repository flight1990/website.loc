<template>
    ...
    <div class="flex items-center justify-between flex-wrap gap-6 mb-10">
        <h1 class="block text-2xl font-bold text-gray-800 sm:text-3xl">{{ review ? 'Редактировать' : 'Создать новый' }} отзыв</h1>
        <inertia-link href="/admin/reviews" class="soft-button-gray">
    ...
    <form class="space-y-6">
        ...
        <div>
            <label for="client" class="block text-sm font-medium mb-2">Имя</label>
            ...
            <p class="text-sm text-rose-500 mb-2">{{ form.errors.client}}</p>
        </div>
        ...
        <button class="solid-button-primary disabled:bg-gray-500" :disabled="form.processing" @click.prevent="submitHandler">{{ review ? 'Обновить' : 'Сохранить' }}</button>
    </form>
